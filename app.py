...
def help(ack, command):
    ...
    for account in accounts:
        if float(account['balance']) > 0.0000000000000000:
            ...
            msg = msg + "{:<8} {:<30} {:<20} {:<20} {:<30}".format(currency, balance, available, ticker_price['price'], float(available) * float(ticker_price['price'])) + '\n'
...
def help(ack, command):
    ...
    for account in accounts:
        if float(account['balance']) > 0.0000000000000000:
            currency = account['currency']
            available = account['available']
            total_deposit = float(pd_list[currency])
            ticker_price = public_client.get_product_ticker(product_id=currency+'-USD')
